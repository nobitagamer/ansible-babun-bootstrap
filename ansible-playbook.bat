...
"%SH%" -c "VAGRANT_PRIV_KEY_DIR=`echo %* | sed '0,\|%CYGWIN_HOME%| s|||' | awk '{print $1}' | sed '0,/--private-key=/ s///' | sed '0,/private_key/ s///'` && echo export VAGRANT_PRIV_KEY_DIRECTORY=$VAGRANT_PRIV_KEY_DIR >> $HOME/.babun_shim_vars"
...
"%SH%" -c "$HOME/ansible/bin/ansible-playbook `echo %* | sed '0,\|%CYGWIN_HOME%/ s///'`"